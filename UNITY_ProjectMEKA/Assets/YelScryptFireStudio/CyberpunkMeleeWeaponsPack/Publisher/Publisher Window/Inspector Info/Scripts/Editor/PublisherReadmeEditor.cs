...
namespace CPMW
{
	[CustomEditor(typeof(CPMWPublisherReadme))]
	[InitializeOnLoad]
	public class PublisherReadmeEditor : Editor
	{
		...
		[MenuItem("Window/Yel Scrypt Fire Studio/Cyberpunk Melee Weapons Pack/Support And Docs")]
		private static CPMWPublisherReadme SelectReadme()
		{
			var ids = AssetDatabase.FindAssets("CPMWPublisherReadme t:CPMWPublisherReadme");
			...
			if (ids.Length == 1)
			{
				...
				return (CPMWPublisherReadme)readmeObject;
			}
			else
			{
		...
		{
			var readme = (CPMWPublisherReadme)target;
